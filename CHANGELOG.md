...
- feat: **Command palette** (`Ctrl+Shift+P`)
  ...
  - **Extensible registry**: a host application registers its own commands with `set_custom_command_palette_entries` / `with_custom_command_palette_entries`, and receives `Message::CommandPaletteAction(id)` when one is run. Entries reuse `ContextMenuItem` rather than a parallel type — the two surfaces describe the same thing (stable `id`, `label`, `shortcut` hint, `enabled` flag), so an action offered in both is declared once and routes through one host handler. The demo app registers eight of its own (open file, save as, new tab, close tab, run, clear log, settings, format document) and now dispatches context-menu and palette actions through a single `handle_app_action`. A `shortcut` hint is display-only — binding the key stays the host's job — so none of the demo's entries carries one: the demo previously advertised `Ctrl+Shift+F` for Format Document, which it never bound and which the editor already reads as Find
  ...
  - The result list shows ten rows at a time and scrolls only when the highlight would otherwise leave that window, rather than on every keypress: walking down the first ten rows leaves the list still, and wrapping from the first row to the last shows the final window instead of leaving a gap below it. The scroll arithmetic works in a row *pitch* (row height plus the one-pixel gap between rows) shared with the list's own height, so an offset always lands exactly on a row boundary however far down the list it is
  - Covered by 27 unit tests (subsequence matching including non-ASCII, registry ordering, omission of unavailable actions, wrapping navigation, filter-resets-highlight, built-in vs host submission, disabled palette, and the scroll-window arithmetic on its own), 2 i18n tests spanning all 18 keys × 8 locales (backed by a new structural check that every locale file defines exactly the keys `en.yml` does — with `fallback = "en"`, no assertion made through `Translations` can see a key missing from a single locale), and 9 interface tests driven through the real widget tree — including a pair asserting that `ArrowDown` and the typed query both reach the palette *without* reaching the buffer or moving the caret, which is the behavior the canvas key listener and the focused query field exist to provide
...
- feat: **Inline color previews**
  ...
  - A literal split by soft wrapping is drawn once, on the visual segment holding its last character. Because any segment of a wrapped line may hold one, every segment needs the whole line's literals — so the scan is memoized per logical line for the duration of a draw pass (`LineLiterals`) instead of being repeated per segment, which kept a line wrapped into sixty segments from being scanned sixty times for the same result
  ...
  - Detection lives in `features/color_preview.rs` as a pure function over a line of text, covered by 19 unit tests (each notation, short-form expansion, uppercase digits and prefixes, percentages, alpha, several literals per line, character-based columns on non-ASCII lines, clamping of out-of-range components, every rejection rule, and the per-line memo — including that it answers from cache rather than rescanning)
